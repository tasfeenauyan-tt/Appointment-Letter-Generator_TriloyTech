...
export async function generateAppointmentDocx(
  ...
  if (shouldIncludeTerms) {
    const termsChildren = buildTermsDocChildren(data, false);
    docChildren.push(...termsChildren);
  }
...
export function buildTermsDocChildren(
  data: AppointmentLetterData,
  ...
  if (isStandalone && data.includeLetterhead) {
    children.push(
      new Paragraph({
        alignment: AlignmentType.CENTER,
        spacing: { after: 40 },
...
export async function generateTermsOnlyDocx(
  data: AppointmentLetterData
): Promise<void> {
  const docChildren = buildTermsDocChildren(data, true);
  ...
  const blob = await Packer.toBlob(doc);
  const nameForFile = (data.employeeFullName || "Employee").replace(/\s+/g, "_");
  const cleanFilename = `Terms_And_Conditions_${nameForFile}_TriloyTech.docx`;
  saveAs(blob, cleanFilename);
}
